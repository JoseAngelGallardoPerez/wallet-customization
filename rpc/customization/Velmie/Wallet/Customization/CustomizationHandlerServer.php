...
/**
 * @see CustomizationHandler
 *
 * Generated from protobuf service <code>velmie.wallet.customization.CustomizationHandler</code>
 */
final class CustomizationHandlerServer implements RequestHandlerInterface
{
    const PATH_PREFIX = '/twirp/velmie.wallet.customization.CustomizationHandler/';
    ...
    public function handle(ServerRequestInterface $req): ResponseInterface
    {
        ...
        $ctx = Context::withPackageName($ctx, 'velmie.wallet.customization');
        $ctx = Context::withServiceName($ctx, 'CustomizationHandler');
        ...
        switch ($req->getUri()->getPath()) {
            case '/twirp/velmie.wallet.customization.CustomizationHandler/GetByKey':
                return $this->handleGetByKey($ctx, $req);
    ...
    private function handleGetByKey(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleGetByKeyJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleGetByKeyProtobuf($ctx, $req);
                break;
    ...
    private function handleGetByKeyJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'GetByKey');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Velmie\Wallet\Customization\Request();
            $in->mergeFromJsonString((string)$req->getBody());

            $out = $this->svc->GetByKey($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling GetByKey. null responses are not supported'));
            }
    ...
    private function handleGetByKeyProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'GetByKey');
